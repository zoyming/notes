...
### 3.4 链接器是怎么通过ELF的位置信息，将多个文件进行连接的呢？
...
在整个过程中，**MMU（内存管理单元）** 扮演了关键角色，它将程序使用的虚拟地址（用户看到的）实时地翻译成物理地址（内存硬件使用的），从而实现了内存保护、进程隔离和虚拟内存等高级功能。

### 总结

从 MIPS 上电到你的程序运行，是一个典型的“接力赛”：

**硬件固化代码 (Boot ROM) -> 功能强大的引导程序 (Bootloader) -> 操作系统内核 (Kernel) -> 用户程序 (Your Program)**

每一阶段都为下一阶段准备更丰富的执行环境，最终将一个存储在静态介质上的二进制文件，变成一个在内存中动态运行的、拥有自己独立地址空间的活生生的进程。
